Shader "SFS/WallpaperMaker_Lite_URP"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _BaseColor;
			float4 _Patern2Color;
			float4 _Patern1Color;
			float _Contrast;
			float _TilingPatern1;
			float _TilingPatern2;
			float _Metallic;
			float _Smoothness;
			...
			sampler2D _Patern1Sprite;
			sampler2D _Patern2Sprite;
						 ...
						 ) : SV_Target
			{
				...
				float temp_output_86_0 = abs( WorldNormal.z );
				float4 appendResult73 = (float4(WorldPosition.x , WorldPosition.y , 0.0 , 0.0));
				float temp_output_83_0 = abs( WorldNormal.x );
				float4 appendResult76 = (float4(WorldPosition.y , WorldPosition.z , 0.0 , 0.0));
				...
				float4 appendResult82 = (float4(WorldPosition.x , WorldPosition.z , 0.0 , 0.0));
				...
				float3 BaseColor = CalculateContrast(_Contrast,lerpResult194).rgb;
